...
import boto3
from process_sales_order_utils_V2 import retrieve_ingested_csv, get_run_number, create_fact_sales_order_dataframe, create_staff_dim_table, create_currency_dim_table, create_counterparty_dim_table, create_design_dim_table, create_date_dim_table, save_to_processed_sales_bucket, create_location_dim_table
...
def test_get_run_num_correctly_gets_largest_run_num():
    ...
    for i in range(11):
        conn.put_object(Body=test_file.getvalue(
        ), Bucket='bosch-deploy-23-12-22-v2-ingest-bucket', Key=f'Run-tracker/run-number{i}.csv')

    result = get_run_number()
    print(conn.list_objects_v2(
        Bucket='bosch-deploy-23-12-22-v2-ingest-bucket', Prefix='Run-tracker'))
    assert result == 10
...
def test_retrieve_ingested_correctly_retrieves_csv_data():
    ...
    test_df = pd.DataFrame({'first_name': ['Ben', 'Scott', 'Omar', 'Hamza', 'Cees'], 'surname': [
                           'Ainger', 'Florence', 'Gonga', 'Mughal', 'Dear'], 'team_name': ['LikeaBosch', 'LikeaBosch', 'LikeaBosch', 'LikeaBosch', 'LikeaBosch']})
    test_csv = StringIO()
    ...
    conn.create_bucket(Bucket='bosch-deploy-23-12-22-v2-ingest-bucket')
    conn.put_object(Body=test_csv.getvalue(
    ), Bucket='bosch-deploy-23-12-22-v2-ingest-bucket', Key='TableName/address/RunNum:4.csv')
...
def test_create_fact_sales_order_dataframe():
    ...
    expected_headers = ['sales_order_id', 'created_date', 'created_time', 'last_updated_date', 'last_updated_time', 'design_id', 'staff_id',
                        'counterparty_id', 'units_sold', 'unit_price', 'currency_id', 'agreed_delivery_date', 'agreed_payment_date', 'agreed_delivery_location_id']
    test_headers = ['sales_order_id', 'created_at', 'last_updated', 'design_id', 'staff_id', 'counterparty_id', 'units_sold',
    ...
    expected_row_1 = [2, datetime.date(2022, 11, 3), datetime.time(14, 20, 52, 186000), datetime.date(2022, 11, 3), datetime.time(
        14, 20, 52, 186000), 3, 19, 8, 42972, 3.94, 2, datetime.date(2022, 11, 7), datetime.date(2022, 11, 8), 8]
...
def test_create_staff_dim_table():
    ...
    expected_row_2 = [2, 'Not Ben', 'Not Ainger',
                      'Secondtest@test.com', 'testers', 'remote']
    test_dict = {}
...
def test_create_currency_dim_table():
    ...
    test_dict = {'currency_id': [1, 2, 3], 'currency_code': [
        'GBP', 'USD', 'EUR'], 'created_at': [1, 2, 3], 'last_updated': [1, 2, 3]}

    expected_headers = ['currency_id', 'currency_code', 'currency_name']
    expected_rows = [[1, 'GBP', 'Great British Pound'],
                     [2, 'USD', 'US Dollar'], [3, 'EUR', 'Euro']]
...
def test_create_couterparty_dim_table():
    ...
    expected_headers = ['counterparty_id', 'counterparty_legal_name', 'counterparty_legal_address_line_1', 'counterparty_legal_address_line_2',
    ...
    result_df = create_counterparty_dim_table(
        test_counterparty_df, test_address_df)
...
def test_create_design_dim_table():
    ...
    expected_headers = ['design_id', 'design_name',
...
def test_date_dim_table():
    ...
    test_rows = [1, '2022-11-03 14:20:52.186', '2022-11-04 14:20:52.186',
                 9, 16, 18, 84754, 2.43, 3, '2022-11-10', '2022-11-09', 4]
...
def test_date_dim_table_does_not_duplicate_dates():
    ...
    test_rows = [1, '2022-11-03 14:20:52.186', '2022-11-03 14:20:52.186',
    ...
    assert result['date_id'].values.tolist() == list(
        set(result['date_id'].values.tolist()))
...
def test_create_location_dim_table():
    ...
    test_headers = ['address_id', 'address_line_1', 'address_line_2', 'district',
                    'city', 'postal_code', 'country', 'phone', 'created_at', 'last_updated']
    test_rows = [[1, 'test house', 'test lane', 'North Testchester', 'Testchester', 'TE5 5ST', 'Testland', '01234567890', 'time', 'later time'], [
        2, 'test cottage', 'test avenue', 'South Testchester', 'Testchester', 'TS5 5ET', 'Testland', '09876543210', 'time', 'later time']]

    expected_headers = ['location_id', 'address_line_1', 'address_line_2',
                        'district', 'city', 'postal_code', 'country', 'phone']
    expected_rows = [[1, 'test house', 'test lane', 'North Testchester', 'Testchester', 'TE5 5ST', 'Testland', '01234567890'], [
        2, 'test cottage', 'test avenue', 'South Testchester', 'Testchester', 'TS5 5ET', 'Testland', '09876543210']]
...
def test_save_to_processed_sales_bucket():
    ...
    key = conn.list_objects_v2(
        Bucket='bosch-deploy-23-12-22-v2-processed-bucket')['Contents'][0]['Key']

    expected_key = 'Sales/location/RunNum:11.parquet'

    result_data = BytesIO()

    conn.download_fileobj('bosch-deploy-23-12-22-v2-processed-bucket',
                          'Sales/location/RunNum:11.parquet', result_data)
